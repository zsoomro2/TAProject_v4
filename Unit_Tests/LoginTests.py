...
class LoginTests(TestCase):
    ...
    def test_getUsername(self):
        ...
        self.assertEqual(user.getUsername('test'), 'test')

    def test_badUsername(self):
        user = Login(username='test1@test.com', password='<PASSWORD>')
        self.assertEqual(user.getUsername(), None, msg="Username not found")

    def test_getPassword(self):
        user = Login(username='test@test.com', password='<PASSWORD>')
        self.assertEqual(user.getPassword('<PASSWORD>'), '<PASSWORD>')
    ...
    def test_goodUser(self):
        user = Login(username='test@test.com', password='<PASSWORD>')
        self.assertEqual(user.findUser('test@test.com', '<PASSWORD>'), 'test@test.com')
